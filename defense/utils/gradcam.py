...
def get_binary_map(mask_pp, perc=0.5):
    ...
    binary_map = cv2.applyColorMap(np.uint8(255 * binary_map.squeeze().cpu()), cv2.COLORMAP_JET)
    binary_map = torch.from_numpy(binary_map).permute(2, 0, 1).float().div(255)
    b, g, r = binary_map.split(1)
    binary_map = torch.cat([r, g, b])
    return binary_map
