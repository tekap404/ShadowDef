...
import copy
from util import renorm_resize
...
class FLGradientInversion(object):
    ...
    def __call__(self, args):
        ...
        targets_in = torch.tensor(y_true, dtype=torch.float, device=device)
        ...
        # model
        self.GAN_model.eval()
        self.GAN_model.mapping.requires_grad_(False)
        self.GAN_model.synthesis.requires_grad_(True)

        # z
        z = torch.randn([args.pretrained_size[0], 512], device=device)
        z.requires_grad_(True)
        z_optimizer = torch.optim.Adam([z], lr=args.z_lr)
        z_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            z_optimizer, 
            milestones=[args.z_iter // 2.667, args.z_iter // 1.6, args.z_iter // 1.142], gamma=0.8)
        ...
        # optimize z
        for iteration_loc in tqdm(range(args.z_iter)):

            c = [np.random.randint(100000) for _ in range(args.pretrained_size[0])]
            c = torch.from_numpy(np.stack(c)).to(device)

            iteration += 1
            z_optimizer.zero_grad()
            self.GAN_model.zero_grad()
            
            inputs = self.GAN_model(z, c)
            inputs = renorm_resize(args, inputs, [-1, 1])
        ...
        z_optimizer.state = collections.defaultdict(dict)
        ...
        self.GAN_model.mapping.requires_grad_(False)
        self.GAN_model.synthesis.requires_grad_(True)
        optimizer = torch.optim.Adam(self.GAN_model.parameters(), lr=args.lr)
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, 
            milestones=[args.iterations // 2.667, args.iterations // 1.6, args.iterations // 1.142], gamma=0.8)

        # z
        z.requires_grad_(True)

        for iteration_loc in tqdm(range(args.iterations)):
    ...
    def create_trainer(self, args, network, targets, criterion, device=None):
        if device is None:
            device = torch.device("cuda")
