...
class chestXray_all_dataset(torchdata.Dataset):
    def __init__(
...
def load_prior(args):
    ...
    img = single_channel_loader(args.template_path)
    img = np.expand_dims(np.array(img), -1).repeat(3, axis=-1)
    cfg = SimpleNamespace(**{})
    cfg.img_size = int(args.pretrained_size[-2])
    cfg.val_size = int(args.pretrained_size[-2])
    cfg.norm_mean = (0.5329, 0.5329, 0.5329)
    cfg.norm_std = (0.2534, 0.2534, 0.2534)
...
def get_partitioned_data_chestXray(cfg, datadir):
...
def load_dataloader(args):
    ...
    net_dataidx_map, df = get_partitioned_data_chestXray(cfg, datadir=cfg.data_dir)
    df_train = pd.read_csv(os.path.join(cfg.data_dir, 'Chest_Xray_train.csv'))
    train_ds = chestXray_dataset(cfg, df=df_train, part_index=net_dataidx_map[args.client_id], train=True, transform=cfg.train_transforms)
...
def load_all_dataloader(args):
    ...
    all_ds = chestXray_all_dataset(cfg, df=df, transform=cfg.train_transforms)
...
def get_target_samples(args):
    ...
    net_dataidx_map, df = get_partitioned_data_chestXray(cfg, datadir=cfg.data_dir)
    loader, _, _ = get_chestXray_dataloader(cfg, df, net_dataidx_map[args.client_id], args.client_id)
    ...
    if args.config != 'prostate_config':
